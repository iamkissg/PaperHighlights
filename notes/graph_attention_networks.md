...
* GAT 由单一的结构叠加得到, 文中称为 graph attentional layer.  该层的输入是一组顶点的特征 $\bold{h}=\left{\vec{h_1}, \vec{h_2}, ..., \vec{h_N}\right}$, 期间通过 self-attention 计算各特征之间的依赖程度, 最终得到新的顶点特征, 作为输出 $\bold{h}^'=\left{\vec{h_1}^', \vec{h_2}^', ..., \vec{h_N}^'\right}$.
* 为获得对输入特征的有效转换, 对顶点特征先进行线性变换, 再计算它们的依赖程度: $e_{ij}=a(\bold{W}\vec{h_i},\bold{W}\vec{h_j})$.
* $a(\cdot, \cdot)$ 由一个全连阶层和 LeakyReLU 组成: $a(\bold{W}\vec{h_i},\bold{W}\vec{h_j})=LeakyReLU(\vec{a}^T \left[\bold{W}\vec{h_i}||\bold{W}\vec{h_j} \right])$ ($||$ 表示 concatenation).
...
* 最后 $\vec{h_i}^'=\sigma(\Sigma_{j\in N_i}\alpha_{ij}\bold{W}\vec{h_j})$,
* 文章发现使用 multi-head attention 有助于稳定学习过程, 并在中间层与输出层使用了不同的聚合策略:
    * 中间层使用 concatenation 来拼接 multi-head 的结果: $\vec{h_i}^'={||}_{k=1}^K \sigma(\Sigma_{j\in N_i}\alpha_{ij}\bold{W}\vec{h_j})$
    * 输出层使用对 multi-head 的结果求平均: $\vec{h_i}^'=\sigma(\frac{1}{K} \Sigma_{k=1}^K \Sigma_{j\in N_i}\alpha_{ij}\bold{W}\vec{h_j})$
